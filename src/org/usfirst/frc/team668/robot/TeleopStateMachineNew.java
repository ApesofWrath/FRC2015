...
public class TeleopStateMachineNew {
...
//	public static int entryState;
	
	public static void stateMachine(boolean isDoneScoring, boolean isScoring, boolean isLift, boolean isManualNew, boolean isReversing) {
	
		if (isManualNew && !(RobotMap.currentStateNew == RobotMap.MANUAL_STATE)) {
			Robot.debugWriter.println("Manual Override State\n");
			Elevator.stop();
			Intake.stop();
			RobotMap.currentStateNew = RobotMap.MANUAL_STATE;
		}
		
//		entryState = RobotMap.currentStateNew;
		switch (RobotMap.currentStateNew){
		
		case RobotMap.INITIAL_STATE:
...
//			}
		...
		//	boolean finish = Elevator.moveP(0);
			...
			if (finish == true) {
				...
				RobotMap.currentStateNew = RobotMap.ELEVATOR_ADJUST_UP_STATE_NEW;
		...
		case RobotMap.ELEVATOR_ADJUST_UP_STATE_NEW:
		
		
			SmartDashboard.putString("State:", "swag");
...
//			boolean moveFinish = Elevator.move(0.5, RobotMap.ELEVATOR_ENCODER_PICKUP_ADJUST); // should be about 10
			...
			if (moveFinish) {
				...
				RobotMap.currentStateNew = RobotMap.CLOSE_INTAKE_STATE;
			}
			
			break;
		
		case RobotMap.CLOSE_INTAKE_STATE:
			
			SmartDashboard.putString("State:", "Close Intake");
			
			Robot.intakePiston.set(DoubleSolenoid.Value.kReverse);
		
			RobotMap.currentStateNew = RobotMap.TOTE_EATING_STATE;
			
			break;
			
			
		case RobotMap.TOTE_EATING_STATE:
			
			SmartDashboard.putString("State:", "Tote Eating");
			
			Intake.spin(.75);
			
			if (!Robot.toteOptic.get()  || Robot.joystickOp.getRawButton(RobotMap.PRETEND_BIN_DETECTED_BUTTON) == true){ // Please note that the optic sensor has been replaced with a button.
				
				Intake.stop();
				
				if (startingAtBottom){
					
					RobotMap.currentStateNew = RobotMap.CLOSE_HUG_PISTON_STATE;
					...
					RobotMap.currentStateNew = RobotMap.ELEVATOR_DOWN_TO_TOTE_STATE;
		...
		case RobotMap.CLOSE_HUG_PISTON_STATE:
			...
			RobotMap.currentStateNew = RobotMap.LIFT_TOTE_STATE;
			
			break;
			
			
		case RobotMap.ELEVATOR_DOWN_TO_TOTE_STATE:
			
			SmartDashboard.putString("State:", "Elevator Down State");
			
			startingAtBottom = false;
			
			boolean pickupHeight = Elevator.moveP(RobotMap.ELEVATOR_ENCODER_PICKUP);
			if (pickupHeight){
				
				Elevator.stop();
				Intake.spin(1);
				
				if (Robot.joystickOp.getRawButton(RobotMap.PRETEND_BIN_DETECTED_BUTTON) == true){
				Intake.stop();
				RobotMap.currentStateNew = RobotMap.LIFT_TOTE_STATE;
				}
				
				if (Robot.joystickOp.getRawButton(5)){
				RobotMap.currentStateNew = RobotMap.SCORING_STATE;
				}
		...
		case RobotMap.LIFT_TOTE_STATE:
			
			SmartDashboard.putString("State:", "Lifting Tote");
			
			boolean liftFinish = Elevator.moveP(RobotMap.ELEVATOR_ENCODER_ONE_TOTE_HEIGHT);
			if (liftFinish){
				Elevator.stop();
				Intake.spin(1);
				Robot.intakePiston.set(DoubleSolenoid.Value.kReverse);
		...
		case RobotMap.SCORING_STATE:
			
			//release the kraken
			SmartDashboard.putString("State:", "SCORING YEEEAHHHHHi");
			...
			if(isDoneScoring){
				...
				RobotMap.currentStateNew = RobotMap.INITIAL_STATE;
		...
		case RobotMap.MANUAL_STATE:
			...
			if (Robot.joystickOp.getRawButton(RobotMap.RETURN_TO_STATE_MACHINE_BUTTON)) {
				...
				RobotMap.currentStateNew = RobotMap.MANUAL_RETURN_STATE;
			}

			break;
			
		case RobotMap.MANUAL_RETURN_STATE:
...
//	if (entryState != RobotMap.currentStateNew) {
//		System.out.println("state change:" + entryState);
//	}
...
//				
//			f	Intake.spin(RobotMap.INTAKE_MOTOR_SPEED);
//				
//				// sets intake piston to open if it isn't open yet
//				// boolean finishThis = Elevator.move(RobotMap.elevatorMotorSpeed, RobotMap.ELEVATOR_ENCODER_PICKUP);
...
//				}
//				
//				if (Robot.joystickOp.getRawButton(4)) {
//					humanToNormal = false;
